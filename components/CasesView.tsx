...
export default function CasesView() {
  const { cases, setActiveCaseId, refreshCases, isLoadingCases } = useAppContext();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [formData, setFormData] = useState({ title: '', procedure: '', client: '', category: 'penale' as any });
  ...
  return (
    <div className="flex-1 overflow-y-auto p-8 bg-navy-950/20">
      <div className="max-w-6xl mx-auto">
        ...
        {isLoadingCases ? (
          <div className="flex justify-center p-20"><Loader2 className="w-8 h-8 text-gold-500 animate-spin" /></div>
        ) : cases.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {cases.map((caseItem) => (
              ...
              >
                ...
                <div className={cn(
                  ...
                  caseItem.status === 'active' ? "bg-gold-500/20 text-gold-500" : 
                  caseItem.status === 'archived' ? "bg-emerald-500/20 text-emerald-500" : "bg-navy-700/50 text-navy-400"
                )}>
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-[10px] font-bold text-navy-500 uppercase tracking-widest mb-1">Proc. n. {caseItem.procedureNumber}</p>
            ...
            {/* New Case Placeholder inline for when cases > 0 */}
            <div 
              onClick={() => setIsModalOpen(true)}
          ...
          /* Empty state for totally empty cases */
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-navy-900 border border-navy-700 w-full max-w-md rounded-3xl p-8 shadow-2xl relative">
            ...
            <form onSubmit={handleCreateCase} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-navy-400 uppercase tracking-widest mb-1.5">Titolo</label>
                <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full bg-navy-950 border border-navy-800 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-gold-500/50" placeholder="Es. Omicidio Rossi" />
              </div>
              <div>
                <label className="block text-xs font-bold text-navy-400 uppercase tracking-widest mb-1.5">N. Procedimento</label>
                <input required type="text" value={formData.procedure} onChange={e => setFormData({...formData, procedure: e.target.value})} className="w-full bg-navy-950 border border-navy-800 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-gold-500/50" placeholder="Es. 1234/23 RGNR" />
              </div>
              <div>
                <label className="block text-xs font-bold text-navy-400 uppercase tracking-widest mb-1.5">Cliente</label>
                <input required type="text" value={formData.client} onChange={e => setFormData({...formData, client: e.target.value})} className="w-full bg-navy-950 border border-navy-800 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-gold-500/50" placeholder="Nome cognome..." />
              </div>
              <div>
                <label className="block text-xs font-bold text-navy-400 uppercase tracking-widest mb-1.5">Materia</label>
                <select value={formData.category} onChange={e => setFormData({...formData, category: e.target.value as any})} className="w-full bg-navy-950 border border-navy-800 rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-gold-500/50">
