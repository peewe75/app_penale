...
export async function POST(req: Request, { params }: { params: Promise<{ caseId: string }> }) {
  ...
  const { time, type, title, desc } = body ?? {};

  if (typeof time !== 'string' || typeof type !== 'string' || typeof title !== 'string' || typeof desc !== 'string') {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      owner_id: userId,
      time,
      type: ['info', 'speaker', 'alert', 'legal'].includes(type) ? type : 'info',
      title,
      desc,
    })
    .select('*')
